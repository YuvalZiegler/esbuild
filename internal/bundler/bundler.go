...
func (s *scanner) addEntryPoints(entryPoints []EntryPoint) []graph.EntryPoint {
	...
	for i, entryPoint := range entryPoints {
		go func(i int, entryPoint EntryPoint) {
			...
			if entryPoint.OutputPath == "" && strings.ContainsRune(entryPoint.InputPath, '*') {
				inputPath := entryPoint.InputPath
				if !strings.HasPrefix(inputPath, "./") && !strings.HasPrefix(inputPath, "../") {
					inputPath = "./" + inputPath
				}
				if pattern := resolver.ParseGlobPattern(inputPath); len(pattern) > 1 {
					if results := s.res.ResolveGlob(entryPointAbsResolveDir, pattern, ast.ImportEntryPoint); results != nil {
						entryPointResolveResults[i] = &results
					}
			...
			if resolveResult != nil {
				if resolveResult.IsExternal {
					...
					entryPointResolveResults[i] = &[]resolver.ResolveResult{*resolveResult}
				}
			} else if !didLogError {
	...
	for i, resolveResults := range entryPointResolveResults {
		...
		for _, resolveResult := range *resolveResults {
			...
			if outputPath == "" {
				...
				// "outext" setting is used instead
